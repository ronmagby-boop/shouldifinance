...
/**
 * Migrated onto CalcShell — the last page that was not.
 *
 * This and should-i-refinance were the two oldest calculators and built their
 * own chrome: nav, breadcrumb, heading, export bar, ad slot, related grid,
 * guide link, disclaimer, and latterly the footer. Eight separate bugs came out
 * of that, each found on its own and each the same failure — something added to
 * CalcShell that never arrived here. The last was the site footer, which every
 * other page picked up automatically and this one needed by hand.
 *
 * Nothing now renders page furniture except CalcShell. There is no second copy
 * left to drift.
 *
 * The state, the handlers and the useMemo body below were moved verbatim from

...
/**
 * Migrated onto CalcShell.
 *
 * This and mortgage-payment were the two oldest calculators and built their own
 * page chrome — nav, breadcrumb, heading, export bar, ad slot, related grid,
 * guide link, disclaimer — instead of passing children to CalcShell like the
 * other 42. That duplication produced seven separate bugs, each found on its
 * own: the related-card cap, copied input components that then drifted, a
 * missing guide link, missing export tagging on the results, an untracked
 * related grid, an untagged finance-costs checkbox, and an ad slot that had to
 * be placed by hand. Every one of them was the same failure — something added
 * to CalcShell that never arrived here.
 *
 * mortgage-payment is still on the old pattern. Search "DOES NOT USE CalcShell".
 *
 * The state, the handlers and the useMemo bodies below were moved verbatim from
 * the old file; the arithmetic was not touched, reordered or tidied.
 */

/* Local to this file rather than imported from components/Inputs, which is one
   of the drifts the old structure caused. Left as-is deliberately: unifying it
   is a separate change with its own diff, not something to bury in a move. */
type Num = number | "";
...
export default function ShouldIRefinance() {
  ...
  return (
    <CalcShell
      ...
      eyebrow="Refinance tools"
      title="Should I refinance?"
      intro="Enter your current loan and a new loan you're considering. We'll compare monthly payments, chart both payoff timelines, and show real interest savings — including any extra payments."
      onExample={loadExample}
      onClear={clearExample}
      relatedSlugs={["refinance-to-pay-off-debt", "extra-payments", "rate-buydown"]}
      disclaimer="Figures are estimates for discussion purposes only. Not a commitment to lend. Actual figures depend on credit, escrow, and underwriting."
    >
